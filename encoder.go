...
// WriteStream writes an event field to the connection. If the provided value
// contains newlines, multiple fields will be emitted. If the stream contains
// invalid UTF-8, it will be coerced into valid UTF-8.
func (e *Encoder) WriteStream(field string, stream io.Reader) error {
	...
	scanner.Split(splitFuncLinePlusUTF8)
...
func splitFuncLinePlusUTF8(data []byte, atEOF bool) (advance int, token []byte, err error) {
...
func findLastCompleteUTF8Position(data []byte) int {
	...
	for n > 0 && lookback < utf8.UTFMax {
		// DecodeLastRune returns the last rune and its size in bytes
	...
	// we've tried, there must be invalid UTF8 characters
	return n
}
...
func validateUTF8Token(data []byte) []byte {
	// If the data is already valid UTF-8, just return a copy
	if utf8.Valid(data) {
		return data
	}
